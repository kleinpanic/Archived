...
### ─── PRIVILEGES ──────────────────────────────────────────────────────────────
if [[ ! -f "$PRIV_FILE" ]]; then
  echo "rody105:1" >"$PRIV_FILE"
  log "Created $PRIV_FILE"
fi
declare -A PRIV
# Load Privileges, normalizing usernames to lowercase 
while IFS=: read -r u lvl; do
  u="${u,,}"
  PRIV["$u"]=$lvl
done <"$PRIV_FILE"
...
# lookup privilege level by lowercase username
user_level(){
  local who="${1,,}"
  echo "${PRIV[$who]:-3}"
}
...
process_output(){
  while IFS= read -r line; do
    # ───────────────────────────────────────────────────────────────────────────
    # 1) Strip out ANY ANSI‐escape/color codes from "$line" → "line_clean"
    #    This sed‐regex removes most common CSI sequences like ESC[0m, ESC[33m, ESC[38;5;…m, etc.
    #
    line_clean="$(
      printf '%s' "$line" \
        | sed -r 's/\x1B\[[0-9;]*[mK]//g'      \
        | sed -r 's/&[0-9A-FK-ORa-fk-or]//g'
    )"
    ...
    # ─ JOIN (match “logged in” or “joined the game”)
    if [[ $line_clean =~ INFO\]:\ ([^[:space:]]+)\[.*\]\ logged\ in ]]; then
      p=${BASH_REMATCH[1]}
      ts=$(date +'%F %T')
      echo "[$ts] $p joined" >>"$CHAT_LOG"
      #rcon_send "say $p Just Joined!"
      discord_notify "🟢 $p joined at $ts"
      continue

    elif [[ $line_clean =~ INFO\]:\ \[[^]]+\]\ ([^[:space:]]+)\ joined\ the\ game ]]; then
      p=${BASH_REMATCH[1]}
      ts=$(date +'%F %T')
      echo "[$ts] $p joined" >>"$CHAT_LOG"
      #rcon_send "say Hello, $p!"
      discord_notify "🟢 $p joined at $ts"
      continue
    fi

    # ─ LEAVE (disconnects or normal leave)
    if [[ $line_clean =~ INFO\]:\ ([^[:space:]]+)\ lost\ connection ]]; then
      p=${BASH_REMATCH[1]}
      ts=$(date +'%F %T')
      echo "[$ts] $p left (lost connection)" >>"$CHAT_LOG"
      #rcon_send "say Goodbye, $p!"
      discord_notify "🔴 $p disconnected at $ts. Unfortunate :'(."
      continue

    elif [[ $line_clean =~ INFO\]:\ ([^[:space:]]+)\ Left\ the\ Server ]]; then
      p=${BASH_REMATCH[1]}
      ts=$(date +'%F %T')
      echo "[$ts] $p left (clean exit)" >>"$CHAT_LOG"
      #rcon_send "say Goodbye, $p!"
      discord_notify "🔴 $p left the server at $ts. Goodbye!"
      continue
    fi

    # ─── ADVANCEMENTS / CHALLENGES / GOALS ──────────────────────────────────────
    if [[ $line_clean =~ INFO\]\:\ ([^[:space:]]+)\ has\ (earned\ the\ advancement|made\ the\ advancement|completed\ the\ challenge|reached\ the\ goal)\:?\ \[([^\]]+)\] ]]; then
    ...
    # ─ SEED FEEDBACK
    if [[ $line_clean =~ Seed:\ \[?(-?[0-9]+)\]? ]] && [[ -n $SEED_REQ ]]; then
      s=${BASH_REMATCH[1]}
    ...
    # ─ ADDSPOT COORDS ─────────────────────────────────────────────────────────
    if [[ -n $SPOT_REQ && $line_clean =~ has\ the\ following\ entity\ data:\ \[([^\]]+)\] ]]; then
    ...
    # ─ CHAT PARSE (supports old "issued server command" and new "[Rank Username] server ..." formats)
    if [[ $line_clean =~ INFO\]:\ \[[^]]+\]\ ([^[:space:]]+)\ issued\ server\ command:\ /\ ?([[:alnum:]]+)(.*)$ ]]; then
      # Old format: "username issued server command: /<command><args>"
      user=${BASH_REMATCH[1]}
      cmd_name=${BASH_REMATCH[2]}
      cmd_args=${BASH_REMATCH[3]}
      msg_full="${cmd_name}${cmd_args}"
      # Log the raw command to chat log
      printf "[%s] <%s> %s\n" "$(date +'%H:%M:%S')" "$user" "$msg_full" >>"$CHAT_LOG"

      # Normalize command string
      raw=$(printf '%s' "$msg_full" | tr '[:upper:]' '[:lower:]' \
    ...
    # ─── NEW CHAT PARSE FOR "[Rank Username] server <cmd>" FORMAT ──────────────
    if [[ $line_clean =~ INFO\]:\ \[.*[[:space:]]([a-zA-Z0-9_]+)\]\ ([sS][eE][rR][vV][eE][rR][[:space:]]+(.*))$ ]]; then
      # New format: "[✦✦✦OWNER✦✦✦ rody105] server help"
      user=${BASH_REMATCH[1]}
      full_msg=${BASH_REMATCH[2]}      # starts with "server "
      cmd_and_args=${BASH_REMATCH[3]}  # everything after "server "
      # Log the raw chat-to-command invocation
      printf "[%s] <%s> %s\n" "$(date +'%H:%M:%S')" "$user" "$full_msg" >>"$CHAT_LOG"

      # Normalize command to lowercase alphanumerics and underscores/space
      raw=$(printf '%s' "$cmd_and_args" | tr '[:upper:]' '[:lower:]' \
            | sed 's/[^a-z0-9_ ~]//g' | xargs)
      prefix=false
      cmd="$raw"
      lvl=$(user_level "$user")
      ...
      if (( lvl<=2 )); then
        case "$cmd" in
          ...
          net)         rcon_send "say NET: $(get_net)" ;;
          heap)        rcon_send "say Heap: $(get_heap)" ;;
          save)        rcon_send save-all ;;
          seed)        SEED_REQ="$user"; rcon_send "seed" ;;
          broadcast\ *)
            msg_b="${cmd#broadcast }"
            rcon_send "say [B] $msg_b"
            discord_notify "📢 Broadcast: $msg_b"
            ;;
          dispatch\ *) rcon_send "${cmd#dispatch }" ;;
          addspot\ *)
            SPOT_REQ="$user"; SPOT_NAME="${cmd#addspot }"
          ...
          gamemode\ *)
            ...
            ;;
          creative)  rcon_send "gamemode creative $user" ;;
          spectator) rcon_send "gamemode spectator $user" ;;
          survival)  rcon_send "gamemode survival $user" ;;
        esac
      fi
      ...
      if (( lvl==1 )); then
        case "$cmd" in
          ...
          give\ me\ *)
            ...
            ;;
          give\ *)
            rest_g=${cmd#give }; tgt_g=${rest_g%% *}; itm_g=${rest_g#* }
            rcon_send "give $tgt_g minecraft:$itm_g"
            rcon_send "say Gave $itm_g to $tgt_g"
            ;;
          addpriv\ *)
            if [[ $cmd =~ ^addpriv[[:space:]]+([^[:space:]]+)([[:space:]]+([1-3]))?$ ]]; then
              newu=${BASH_REMATCH[1],,}
...
while true; do
  ...
  discord_notify "⚠️ Crashed. Uh Oh."
  sleep "$RESTART_DELAY"
done
