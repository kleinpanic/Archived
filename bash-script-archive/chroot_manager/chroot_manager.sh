...
function mount_filesystems() {
    # now also mounts /run
    local dirs=(dev proc sys tmp run)
    for d in "${dirs[@]}"; do
    ...
    # ─── NEW: bind DNS resolver so SSL & hostname lookups work ───
    if ! mountpoint -q "${CHROOT_DIR}/etc/resolv.conf"; then
        log_info "Mounting /etc/resolv.conf to ${CHROOT_DIR}/etc/resolv.conf..."
        sudo mkdir -p "${CHROOT_DIR}/etc"
        if ! sudo mount --bind /etc/resolv.conf "${CHROOT_DIR}/etc/resolv.conf"; then
            log_error "Error mounting /etc/resolv.conf"
            exit 1
        fi
    else
        log_debug "${CHROOT_DIR}/etc/resolv.conf is already mounted."
    fi
}

function unmount_filesystems() {
    # now also unmounts run
    local dirs=(dev/pts dev proc sys tmp run)
    local any_mounted=0
    ...
    # ─── NEW: unmount /etc/resolv.conf ───
    if mountpoint -q "${CHROOT_DIR}/etc/resolv.conf"; then
        log_info "Unmounting ${CHROOT_DIR}/etc/resolv.conf..."
        if ! sudo umount "${CHROOT_DIR}/etc/resolv.conf"; then
            log_error "Error unmounting /etc/resolv.conf"
        fi
    else
        log_debug "${CHROOT_DIR}/etc/resolv.conf is not mounted."
    fi
